...
def search():
    ...
    if activity=="FamilyNight":
        ...
        res = db.session.execute("""SELECT movie.id, movie.title, movie.poster, movie.rated, movie.rating from \
                                    ...
                                    group by movie.id, movie.title, movie.poster,movie.rated, movie.rating order by random(), movie.rating limit 7;""")
    ...
    if activity=="SurpriseMe":
        res=db.session.execute("""SELECT movie.id, movie.title, movie.poster, movie.rated, movie.rating from \
                                  genre join movie_genre on (genre.id=movie_genre.genre_id) join movie on (movie_genre.movie_id=movie.id)\
                                  order by random(),movie.rating  limit 7;""")
...
def searchMovie():
    ...
    movie=movie.title()
    res=db.session.execute("""SELECT movie.id, movie.title, movie.poster, movie.rated, movie.rating from movie where movie.title like'%%%s%%' limit 7;"""%movie)
    res=res.fetchall()
