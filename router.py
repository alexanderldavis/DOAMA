...
def search():
    ...
    if activity=="NerdNight":
        res=db.session.execute("""SELECT movie.id, movie.title, movie.poster, movie.rated, movie.rating from \
                                  genre join movie_genre on (genre.id=movie_genre.genre_id) join movie on (movie_genre.movie_id=movie.id)\
                                  where ((genre.genre='Sci-Fi') or (genre.genre='Mystery')) \
                                  group by movie.id, movie.title, movie.poster,movie.rated, movie.rating order by random(),movie.rating  limit 12;""")
        activity="Nerd Night"
...
def searchMovie():
    ...
    #     movie=movie.replace("'","''")
    # movie=movie.title()
    res=db.session.execute("""SELECT movie.id, movie.title, movie.poster, movie.rated, movie.rating from movie where upper(movie.title) like'%%%s%%' order by random() limit 12;"""%(movie.upper()))
    res=res.fetchall()
    count=len(res)
...
def goodFor():
    ...
    if rating != '':
        rating = "'"+rating+"'"
        res=db.session.execute("""SELECT movie.id, movie.title, movie.poster, movie.rated, movie.rating from movie JOIN movie_genre ON (movie.id = movie_genre.movie_id) where movie_genre.genre_id = %s and ((movie.rated = %s) or (movie.rated='PG')) order by random() limit 12;"""%(genre, rating))
    else:
    ...
    res=res.fetchall()
    count=len(res)
    genreList=db.session.execute("""SELECT genre from genre where genre.id=%s"""%genre)
    (genrename,)=genreList.fetchall()[0]
    return render_template('searchresults.html',movieList=res,activity=group+" who like "+genrename,count=count)
...
def addMovieToDb():
    ...
    res=db.session.execute("""SELECT count(*) from movie where upper(title) like '%%%s%%' """%(movieName.upper()))
