...
def goodFor():
    ...
    res=res.fetchall()
    count=len(res)
    genrename=db.session.execute("""SELECT genre from genre where genre.id=%d"""%d )
    return render_template('searchresults.html',movieList=res,activity="Good for "+group+" who like "+genrename,count=count)
